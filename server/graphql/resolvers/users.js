      ...
      try {
        ...
        users = users.map((Newuser) => {
          const latestMessage = AlluserMessages.find(
            (m) =>
              m.sender === Newuser.username || m.receiver === Newuser.username
          );
    ...
    addUser: async (parent, args) => {
      ...
      try {
        ...
        if (confirmPassword.trim() === "")
          errors.confirmPassword = "Please repeat the Same Password";
        ...
        if (err.code === 11000 && err.keyValue.username)
          err.username = "that username is already used";
        if (err.code === 11000 && err.keyValue.email)
          err.email = "that Email is already used";
        if (err.name === "ValidationError")
          err.email = "Please Entre a valide Email";
